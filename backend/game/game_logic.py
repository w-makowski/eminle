...
def process_guess(correct_song, user_guess):
    """Processes the user's guess and returns hints for each attribute."""
    album_hint, album_color = get_hints_and_colours(user_guess.album.album_number, correct_song.album.album_number, is_numeric=True)
    track_hint, track_color = get_hints_and_colours(user_guess.track_number, correct_song.track_number, is_numeric=True)
    track_length_hint, track_length_color = get_hints_and_colours(
        get_total_seconds_from_track_length(user_guess.track_length),
    ...
    guessed_features = set(re.split(r"[,|and|&]", user_guess.features or ''))
    feature_color = 'green' if correct_features == guessed_features else ('yellow' if correct_features & guessed_features else "grey")
    _, song_color = get_hints_and_colours(user_guess.song_name.lower(), correct_song.song_name.lower())

    return {
        'song_name': user_guess.song_name,
        'song_name_color': song_color,
        'album': {'value': user_guess.album.name, 'hint': album_hint, 'color': album_color},
        'track_number': {'value': user_guess.track_number, 'hint': track_hint, 'color': track_color},
        'track_length': {'value': user_guess.track_length, 'hint': track_length_hint, 'color': track_length_color},
        'features': {'value': user_guess.features or 'None', 'color': feature_color}
    }
